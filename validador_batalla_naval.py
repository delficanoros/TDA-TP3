    ...
    if long_barco == 1:
        ...
        for i in range(long_barco):
            if solucion[x][y+i] == 0 or casilleros_visitados[x][y+i] or y + i >= len(solucion[0]):
                return False
            if hay_adyacentes(solucion, casilleros_visitados, x, y+i):
                return False
        ...
        for i in range(long_barco):
            if solucion[x+i][y] == 0 or casilleros_visitados[x+i][y] or x + i >= len(solucion):
                return False
            if hay_adyacentes(solucion, casilleros_visitados, x+i, y):
                return False
...
def validador(matriz, barcos, restricciones_fil, restricciones_col, solucion):
    ...
    for i in range(fils):
        # la cantidad de casilleros ocupados por la fila k es distinta a la especificada en la restriccion
        if sum(solucion[i]) != restricciones_fil[i]:
            return False
    ...
    # verifico que todos los barcos solicitados esten en la solucion
    barcos_no_visitados = barcos[:]

    for i in range(fils):
        for j in range(cols):
            if solucion[i][j] == 1 and not casilleros_visitado[i][j]:
                ...
                if j+1 < fils and solucion[i][j+1] == 1:
                    ...
                    while j + long_barco < fils and solucion[i][j+long_barco] == 1:
                        long_barco += 1
